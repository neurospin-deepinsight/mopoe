# coding=utf-8
# Taken from https://github.com/google/compare_gan/blob/master/compare_gan/
# src/prd_score_test.py
...
class PRDTest(unittest.TestCase):
    ...
    def test_compute_prd_from_embedding_mismatch_num_samples_should_fail(self):
        ...
        with self.assertRaises(ValueError):
            prd.compute_prd_from_embedding(np.array([[0], [0], [1]]), np.array(
                [[0], [1]]), num_clusters=2, enforce_balance=True)
    ...
    def test_compute_prd_from_embedding_mismatch_num_samples_should_work(self):
        ...
        try:
            prd.compute_prd_from_embedding(np.array([[0], [0], [1]]), np.array(
                [[0], [1]]), num_clusters=2, enforce_balance=False)
        except ValueError:
            self.fail(
                'compute_prd_from_embedding should not raise a ValueError '
                'when enforce_balance is set to False.')
