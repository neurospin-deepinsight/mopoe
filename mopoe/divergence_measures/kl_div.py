...
def calc_kl_divergence(mu0, logvar0, mu1=None, logvar1=None, norm_value=None):
    if mu1 is None or logvar1 is None:
        ...
        KLD = -0.5 * (torch.sum(1 - logvar0.exp() / logvar1.exp() -
                      (mu0 - mu1).pow(2) / logvar1.exp() + logvar0 - logvar1))
    if norm_value is not None:
        KLD = KLD / float(norm_value)
    return KLD


def calc_gaussian_scaling_factor(
        PI,
        mu1,
        logvar1,
        mu2=None,
    ...
    if mu2 is None or logvar2 is None:
        # print('S_11: ' +
        #       str(torch.sum(1/((2*PI*(logvar1.exp() + 1)).pow(0.5)))))
        # print('S_12: ' +
        #       str(torch.sum(torch.exp(-0.5*(mu1.pow(2)/(logvar1.exp()+1))))))
        S_pre = (1 / (2 * PI).pow(d / 2)) * \
            torch.sum((logvar1.exp() + 1), dim=1).pow(0.5)
        S = S_pre * \
            torch.sum((-0.5 * (mu1.pow(2) / (logvar1.exp() + 1))).exp(), dim=1)
        S = torch.sum(S)
    else:
        # print('S_21: ' +
        #       str(torch.sum(1/((2*PI).pow(d/2)*
        #           (logvar1.exp()+logvar2.exp()).pow(0.5)))))
        # print('S_22: ' +
        #       str(torch.sum(torch.exp(-0.5 *
        #           ((mu1 - mu2).pow(2) / (logvar1.exp() + logvar2.exp()))))))
        S_pre = torch.sum(1 / ((2 * PI).pow(d / 2) *
                               (logvar1.exp() + logvar2.exp())),
                          dim=1).pow(0.5)
        S = S_pre * torch.sum(
            torch.exp(-0.5 * ((mu1 - mu2).pow(2) /
                              (logvar1.exp() + logvar2.exp()))), dim=1)
        S = torch.sum(S)
    if norm_value is not None:
...
# def calc_kl_divergence_lb_gauss_mixture(flags, index, mu1, logvar1, mus,
#                                         logvars, norm_value=None):
...
#             kld = 0.0
#         else:
#             kld = calc_kl_divergence(mu1, logvar1, mus[k], logvars[k],
#                                      norm_value=norm_value)
#         klds[k+1] = kld
#     kld_mixture = klds.mean()
#     return kld_mixture

def calc_kl_divergence_lb_gauss_mixture(
        flags,
        index,
        mu1,
        logvar1,
...
def calc_kl_divergence_ub_gauss_mixture(
        flags,
        index,
        mu1,
        logvar1,
    ...
    for k in range(0, len(mus)):
        if index == k:
            ...
            kl_div = calc_kl_divergence(
                mu1, logvar1, mus[k], logvars[k], norm_value=norm_value)
            print('kl div ' + str(k) + ': ' + str(kl_div))
            denom += w_modalities[k + 1] * \
                torch.min(torch.Tensor([kl_div.exp(), 100000]))
