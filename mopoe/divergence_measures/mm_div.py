...
from mopoe.divergence_measures.kl_div import (
    calc_kl_divergence_lb_gauss_mixture)
from mopoe.divergence_measures.kl_div import (
    calc_kl_divergence_ub_gauss_mixture)
from mopoe.divergence_measures.kl_div import calc_entropy_gauss
...
def calc_alphaJSD_modalities_mixture(
        m1_mu, m1_logvar, m2_mu, m2_logvar, flags):
    klds = torch.zeros(2)
...
def calc_modality_divergence(m1_mu, m1_logvar, m2_mu, m2_logvar, flags):
    if flags.modality_poe:
        ...
        for i in range(
                1, len(mus)):  # CAREFUL: index starts from one, not zero
