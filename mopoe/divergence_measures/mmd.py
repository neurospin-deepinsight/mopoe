def mmd_loss(z_tilde, z, z_var):
    ...
    n = z.size(0)
    im_kernel_z_z = im_kernel_sum(
        z, z, z_var, exclude_diag=True).div(n * (n - 1))
    im_kernel_ztilde_ztilde = im_kernel_sum(
        z_tilde, z_tilde, z_var, exclude_diag=True).div(n * (n - 1))
    im_kernel_z_ztilde = im_kernel_sum(
        z, z_tilde, z_var, exclude_diag=False).div(
        n * n).mul(2)
...
def im_kernel_sum(z1, z2, z_var, exclude_diag=True):
    ...
    Args:
        ...
        exclude_diag (bool): whether to exclude diagonal kernel measures before
            sum it all.
    """
