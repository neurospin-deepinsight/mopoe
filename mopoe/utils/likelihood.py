...
def gaussian_log_pdf(x, mu, logvar):
    ...
    """
    global LOG2PI
    log_pdf = -0.5 * LOG2PI - logvar / 2. - \
        torch.pow(x - mu, 2) / (2. * torch.exp(logvar))
...
def log_marginal_estimate(
    ...
    if style is not None:
        ...
        style_log_q_z_given_x_2d = gaussian_log_pdf(
            z_style, mu_style, logvar_style)
    ...
    log_p_x_given_z_2d = likelihood.log_prob(image).view(
        batch_size * n_samples, -1).sum(dim=1)
    content_log_q_z_given_x_2d = gaussian_log_pdf(
        z_content, mu_content, logvar_content)
    ...
    if dynamic_prior is None:
        ...
        log_p_z_2d_content = gaussian_log_pdf(
            z_content, mu_prior, logvar_prior)

    if style is not None:
        log_p_z_2d = log_p_z_2d_style + log_p_z_2d_content
        log_q_z_given_x_2d = (
            style_log_q_z_given_x_2d + content_log_q_z_given_x_2d)
    else:
...
def log_joint_estimate(
    ...
    if styles is not None:
        ...
        for key in styles.keys():
            if styles[key] is not None:
                ...
                style_m_log_q_z_given_x_2d = gaussian_log_pdf(
                    z_style_m, mu_style_m, logvar_style_m)
    ...
    for k, key in enumerate(styles.keys()):
        ...
        if len(d_shape) == 3:
            ...
            batch_d = batch_d.view(
                batch_size * n_samples, d_shape[-2], d_shape[-1])
        elif len(d_shape) == 4:
            batch_d = batch_d.unsqueeze(0).repeat(n_samples, 1, 1, 1, 1)
            batch_d = batch_d.view(
                batch_size * n_samples, d_shape[-3], d_shape[-2], d_shape[-1])
        lhood = likelihoods[key]
        log_p_x_given_z_2d = lhood.log_prob(batch_d).view(
            batch_size * n_samples, -1).sum(dim=1)
