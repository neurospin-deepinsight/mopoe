...
class BaseMMVae(ABC, nn.Module):
    ...
    def set_fusion_functions(self):
        weights = utils.reweight_weights(
            torch.Tensor(self.flags.alpha_modalities))
    ...
    def poe_fusion(self, mus, logvars, weights=None):
        if (self.flags.modality_poe or mus.shape[0] ==
            ...
            mus = torch.cat(
                (mus, torch.zeros(
                    1, num_samples, self.flags.class_dim).to(
                    self.flags.device)), dim=0)
            logvars = torch.cat(
                (logvars, torch.zeros(
                    1, num_samples, self.flags.class_dim).to(
                    self.flags.device)), dim=0)
    ...
    def forward(self, input_batch, sample_latents=True):
        ...
        for m, m_key in enumerate(self.modalities.keys()):
            if m_key in input_batch.keys():
                ...
                if (self.flags.factorized_representation and sample_latents and
                        self.flags.style_dim[m] > 0):
                    m_s_embeddings = self.reparameterize(
                        mu=m_s_mu, logvar=m_s_logvar)
                else:
                    m_s_embeddings = m_s_mu
                m_rec = self.lhoods[m_key](*self.decoders[m_key](
                    m_s_embeddings, class_embeddings))
                ...
                for m, mod in enumerate(mods):
                    if mod.name in input_batch.keys():
                        mus_subset = torch.cat(
                            (mus_subset, enc_mods[mod.name][0].unsqueeze(0)),
                            dim=0)
        ...
        if self.flags.modality_jsd:
            ...
            logvars = torch.cat(
                (logvars, torch.zeros(
                    1, num_samples, self.flags.class_dim).to(
                    self.flags.device)), dim=0)
        # weights = (1/float(len(mus)))*torch.ones(len(mus)).to(
        #   self.flags.device)
        weights = (1 / float(mus.shape[0])) * \
            torch.ones(mus.shape[0]).to(self.flags.device)
    ...
    def get_random_styles(self, num_samples):
        styles = dict()
        for k, m_key in enumerate(self.modalities.keys()):
            if (self.flags.factorized_representation and
                    self.flags.style_dim[k] > 0):
                mod = self.modalities[m_key]
