...
def generate_random_samples_plots(exp, epoch):
    ...
    for k, m_key in enumerate(mods.keys()):
        ...
        p = plot.create_fig(
            fn, mod_plot, 10, save_figure=exp.flags.save_figure)
        ...
        for l, m_key_out in enumerate(mods.keys()):
            ...
            for i in range(len(samples)):
                for j in range(len(samples)):
                    i_batch_s = {
                        mod_out.name: samples[i][
                            mod_out.name].unsqueeze(0)}
                    i_batch_c = {
                        mod_in.name: samples[i][
                            mod_in.name].unsqueeze(0)}
                    ...
                    swap_out = mod_out.plot_data(
                        swap_sample[mod_out.name].squeeze(0))
    ...
    for k, s_key in enumerate(subsets.keys()):
        ...
        if num_mod_s == M:
            ...
            for l, m_key_out in enumerate(mods.keys()):
                mod_out = mods[m_key_out]
                rec = torch.zeros(
                    exp.plot_img_size, dtype=torch.float32).repeat(
                    100 + M * 10, 1, 1, 1)
            ...
            for i in range(len(samples)):
                for j in range(len(samples)):
                    ...
                    for l, m_key_out in enumerate(mods.keys()):
                        ...
                        if exp.flags.factorized_representation:
                            style[
                                mod_out.name] = random_styles[
                                mod_out.name][i].unsqueeze(0)
                        else:
